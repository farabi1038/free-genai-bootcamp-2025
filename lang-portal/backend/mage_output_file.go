 ...
 `[1:], _filepath.Base(os.Args[0]))
	...
	if args.Help {
		...
		switch _strings.ToLower(args.Args[0]) {
			case "initdb":
				_fmt.Println("InitDB initializes the SQLite database file")
				_fmt.Println()
				...
				return
			case "migratedb":
				_fmt.Println("MigrateDB runs database migrations")
				_fmt.Println()
				...
				return
			case "seeddb":
				_fmt.Println("SeedDB imports seed data into the database")
				_fmt.Println()
				...
				return
			case "setupdb":
				_fmt.Println("SetupDB runs the complete database setup process")
				_fmt.Println()
		...
		switch _strings.ToLower(target) {
			...
			case "initdb":
				...
				if expected > len(args.Args) {
					...
					logger.Printf("not enough arguments for target \"InitDB\", expected %v, got %v\n", expected-1, len(args.Args)-1)
					os.Exit(2)
				}
				if args.Verbose {
					logger.Println("Running target:", "InitDB")
				}
			...
			case "migratedb":
				...
				if expected > len(args.Args) {
					// note that expected and args at this point include the arg for the target itself
					// so we subtract 1 here to show the number of args without the target.
					logger.Printf("not enough arguments for target \"MigrateDB\", expected %v, got %v\n", expected-1, len(args.Args)-1)
					os.Exit(2)
				}
				if args.Verbose {
					logger.Println("Running target:", "MigrateDB")
				}
			...
			case "seeddb":
				...
				if expected > len(args.Args) {
					// note that expected and args at this point include the arg for the target itself
					// so we subtract 1 here to show the number of args without the target.
					logger.Printf("not enough arguments for target \"SeedDB\", expected %v, got %v\n", expected-1, len(args.Args)-1)
					os.Exit(2)
				}
				if args.Verbose {
					logger.Println("Running target:", "SeedDB")
				}
			...
			case "setupdb":
				...
				if expected > len(args.Args) {
					// note that expected and args at this point include the arg for the target itself
					// so we subtract 1 here to show the number of args without the target.
					logger.Printf("not enough arguments for target \"SetupDB\", expected %v, got %v\n", expected-1, len(args.Args)-1)
					os.Exit(2)
				}
				if args.Verbose {
					logger.Println("Running target:", "SetupDB")
				}
